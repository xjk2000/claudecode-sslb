...
TEMPLATE_DIR="${CLAUDE_PLUGIN_ROOT:-${HOME}/.claude/docs/huangdi}/templates/huangdi"
...
cmd_init() {
    ...
    # 三省中枢
    mkdir -p "$HUANGDI_DIR/政事堂"
    mkdir -p "$HUANGDI_DIR/秘书省"
    mkdir -p "$HUANGDI_DIR/弘文馆"
    # 六部文档库
    mkdir -p "$HUANGDI_DIR/考功司"
    mkdir -p "$HUANGDI_DIR/籍账库"
    mkdir -p "$HUANGDI_DIR/职方司"
    mkdir -p "$HUANGDI_DIR/都官司"
    mkdir -p "$HUANGDI_DIR/礼部精舍"
    mkdir -p "$HUANGDI_DIR/营缮司"
    # 五监文档库
    mkdir -p "$HUANGDI_DIR/将作图谱库"
    mkdir -p "$HUANGDI_DIR/百工署"
    mkdir -p "$HUANGDI_DIR/甲弩坊署"
    mkdir -p "$HUANGDI_DIR/河渠书库"
    mkdir -p "$HUANGDI_DIR/经籍库"
